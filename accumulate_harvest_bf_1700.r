...
################# luh2elm single-cell grid mapping function
# alternative version of luh2elm for area based harvest rate
luh2elm_cell_average <- function(cell, lonW, lonE, latS, latN, p4string, type_names, raster_stack)	{
  # get the current out cell bounds
  # this assumes that cell starts at 0 to eliminate a calc inside
  lt = trunc(cell / nlon_out) + 1
  ln = cell %% nlon_out + 1
...
mcout = mclapply(c(0:(ncells_out-1)), function(i) luh2elm_cell_average(cell = i, lonW, lonE, latS, latN, PROJ4_STRING, luh2_bioharv_names, harvest_in_rast_stack), mc.cores = 1)
